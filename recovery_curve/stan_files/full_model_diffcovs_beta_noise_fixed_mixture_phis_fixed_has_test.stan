...
model{
	...
	for(i in 1:N){
	      ...
	      pis[i] ~ beta(1,1); # diffuse beta prior on mixture probability
	}
	...
	for(i in 1:N){
	      for(j in 1:ls[i]){
		    ...
		    if (vs[c] < 0.02){
		       	 increment_log_prob(log(pis[i] * (1-R)));		    
		    }
		    else if (vs[c] > 1.0 - 0.02){
		    	 increment_log_prob(log(pis[i] * R));
		    }
		    else{ 
		    	 increment_log_prob(log(1.0-pis[i]) + beta_log(vs[c],1.0 + (s_m * m), 1.0 + (s_m * (1.0 - m))));
		    }
		    c <- c + 1;
	      }
	}
